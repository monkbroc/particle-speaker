...
//   Changes by khpeterson
//     - added option for stereo playback via DAC1 + DAC2
//     - samples packed into uint32_t's according to DAC_DHR12LD register
//     - DAC2[11:0] xxxx DAC1[11:0] xxxx
...
Speaker::Speaker(uint16_t bufferSize, bool dblBuffer)
    : Speaker(bufferSize, dblBuffer, false)
{ }

Speaker::Speaker(uint16_t bufferSize, bool dblBuffer, bool stereo)
    ...
    stereo(stereo)
{
    ...
    for (int i = 0; i < numBuffers; i++) {
		if (!stereo) {
			uint16_t *monoBuffer = new uint16_t[bufferSize];
			buffer[i] = monoBuffer;
			std::fill(monoBuffer, &monoBuffer[bufferSize], 0);
		} else {
			uint32_t *stereoBuffer = new uint32_t[bufferSize];
			buffer[i] = stereoBuffer;
			std::fill(stereoBuffer, &stereoBuffer[bufferSize], 0);
		}
    }
    if (!dblBuffer)
      buffer[1] = buffer[0];
...
Speaker::Speaker(uint32_t *buffer, uint16_t bufferSize) 
    : Speaker(buffer, buffer, bufferSize)
{ }

Speaker::Speaker(uint32_t *buffer0, uint32_t *buffer1, uint16_t bufferSize)
    : bufferSize(bufferSize) ,
    lastBuffer(0xFF),
    audioFrequency(8000),
    privateBuffer(false),
...
Speaker::~Speaker()
{
    if (privateBuffer) {
        for (int i = 0; i < numBuffers; i++) {
			if (!stereo) {
				delete (uint16_t *)buffer[i];
			} else {
				delete (uint32_t *)buffer[i];
			}
        }
    }
}
...
void Speaker::setupHW(uint16_t audioFrequency)
{
	...
	if (!stereo) {
		...
		DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t) &_DAC->DHR12LD;
	}
...
uint16_t *Speaker::getBuffer()
{
    ...
    return currentBuffer() ? (uint16_t *)buffer[0] : (uint16_t *)buffer[1];
}

uint32_t *Speaker::getStereoBuffer()
{
    /* Return alternate buffer for DMA1 Stream5 double buffering */
    return currentBuffer() ? (uint32_t *)buffer[0] : (uint32_t *)buffer[1];
}
